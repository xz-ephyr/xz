...
export default function ProjectItem({ project, onDelete }: ProjectItemProps) {
  ...
  return (
    <div className="mb-1" onMouseLeave={() => setShowMenu(false)}>
      <div
        className="flex items-center gap-3 p-2 hover:bg-sidebar-accent rounded-[8px] cursor-pointer group relative"
      ...
      >
        ...
        {showMenu && (
            <div className="absolute right-0 top-full mt-1 w-32 bg-popover border border-border rounded-xl shadow-lg py-1 z-50">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onDelete(project.id);
        ...
        <div className="ml-4 border-l border-border pl-2 mt-1 space-y-1">
          {sessions.map((session) => (
            <div
              ...
              className={`text-sm p-2 hover:bg-sidebar-accent rounded-[8px] flex items-center gap-3 group cursor-pointer active:scale-[0.99] transition-transform ${uuid === session.id ? 'bg-sidebar-accent text-sidebar-foreground font-medium' : 'text-sidebar-foreground/60'}`}
            >
