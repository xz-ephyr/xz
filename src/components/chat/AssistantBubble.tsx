    ...
    <div className="mb-6 w-full group/bubble">
      <div className="text-base px-4 py-4 break-words flex flex-col gap-2">
        {hasOtherPendingTool && (
          <div className="flex items-center gap-2 text-muted-foreground">
            {pendingTools.filter((ti) => ti.toolName !== 'writeArtifact').map((ti) => {
              ...
              return (
                <div key={ti.toolCallId} className="flex items-center gap-1.5 px-2 py-1 bg-muted rounded-[6px] text-xs font-medium text-muted-foreground border border-border">
                  <span className="thinking-shimmer-text capitalize">
                    {ti.state === 'result' ? 'done' : 'running'}
                  </span>
                  {fileName && <span className="text-muted-foreground/70 font-mono truncate max-w-[160px]">{fileName}</span>}
                </div>
              );
            })}
          </div>
              ...
              {isReasoningOpen && (
                <div className="flex flex-col items-center shrink-0">
                  <div className="flex items-center justify-center w-5 h-5">
                    <HugeiconsIcon icon={Clock01Icon} size={14} className="text-muted-foreground/70" />
                  </div>
                  <div className="w-0.5 flex-1 min-h-4 bg-accent mt-1" />
                </div>
              )}
        ...
        {phase !== 'idle' ? (
          <>
            {streamedIntention && (
              <div className="font-normal text-foreground stagger-item stagger-0">
                <MarkdownMessage content={streamedIntention} />
              </div>
            )}
            {(phase === 'shimmer' || phase === 'explanation' || phase === 'done') && hasWriteArtifact && (
              <div className="flex items-center gap-2 text-muted-foreground stagger-item stagger-1">
            ...
            {(phase === 'explanation' || phase === 'done') && streamedExplanation && (
              <div className="font-normal text-foreground stagger-item stagger-2">
