    ...
    >
          ...
          {isEditing ? (
            <form onSubmit={submitRename} onClick={(e) => e.stopPropagation()}>
              <input
                ...
                className="w-full bg-background border border-border rounded px-2 py-1 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-ring"
              />
            </form>
          ) : (
      ...
      <div className="shrink-0 relative" ref={menuRef}>
        <button
          onClick={toggleMenu}
          className={cn(
            'p-1.5 hover:bg-accent rounded-md text-muted-foreground transition-all opacity-0 group-hover:opacity-100',
        ...
        {isMenuOpen && (
          <div className="absolute right-0 mt-1 w-40 bg-background border border-border rounded-xl shadow-xl py-1.5 z-50 animate-in fade-in zoom-in duration-100 origin-top-right">
            <button
              onClick={handleRename}
              className="w-full flex items-center gap-2 px-3 py-2 text-sm text-card-foreground hover:bg-muted transition-colors"
            >
              <HugeiconRenderer icon={PencilEdit02Icon} size={16} className="text-muted-foreground/70" />
              <span>Rename</span>
            </button>
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2 text-sm text-card-foreground hover:bg-muted transition-colors"
            >
              <HugeiconRenderer icon={ArchiveIcon} size={16} className="text-muted-foreground/70" />
            ...
            >
              <HugeiconRenderer icon={Delete02Icon} size={16} className="text-destructive/60" />
              <span>Delete</span>
            </button>
          </div>
        )}
    ...
    <div className="flex-1 bg-background overflow-y-auto thin-scrollbar">
      <div className="mx-auto px-6 py-12" style={{ maxWidth: 'min(800px, 100%)' }}>
        ...
        <div className="flex flex-col gap-6 mb-8">
          ...
          {/* Tab Switcher */}
          <div className="flex p-1 bg-muted rounded-[6px] w-fit">
            <button
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex-1 group">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground/70 group-focus-within:text-foreground transition-colors">
              ...
              <input
                ...
                className="w-full bg-muted border border-border rounded-2xl py-3 pl-12 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-ring/10 focus:border-border transition-all placeholder:text-muted-foreground/70"
              />
            </div>
            ...
            <div className="relative" ref={filterRef}>
              ...
              {isFilterOpen && (
                <div className="absolute right-0 mt-2 w-52 bg-background border border-border rounded-2xl shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200 origin-top-right">
                  <div className="px-4 py-2 text-[11px] font-bold text-muted-foreground/70 uppercase tracking-widest">
                    Filter by
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="w-full flex items-center justify-between px-4 py-3 text-sm text-card-foreground hover:bg-muted transition-colors group/item"
                  >
                    <span
                      className={cn(
                        'font-medium',
        ...
        <div className="space-y-0.5 overflow-visible">
          {filteredChats.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center animate-in fade-in duration-500">
              <div className="w-20 h-20 bg-muted rounded-full flex items-center justify-center text-muted-foreground/20 mb-6 border border-border/50 shadow-sm">
              ...
              {!searchQuery && filter === 'active' && (
                <Link
                  to="/thread/new"
                  className="mt-8 px-6 py-2.5 bg-foreground text-background text-sm font-semibold rounded-full hover:bg-foreground/90 transition-all hover:shadow-lg active:scale-95"
                >
                  Start a new thread
                </Link>
              )}
